...
master_data{1} = load('cdye_ratio128x_lowInt.mat');
master_data{2} = load('cdye_ratio128x_medInt.mat');
master_data{3} = load('cdye_ratio128x_hiInt.mat');
...
for data_int_ind = 1:3
    fprintf('\n\n %s int data\n',data_int_print_cell{data_int_ind}),
    for pho_frac_ind = 1:8
        jmax = 1;
        cyclesmax = 100;
        tseries = 0;
        exptmax= 1;
        [input] = ini_input(cyclesmax,exptmax,jmax); %Initialize input structure
        comment =  sprintf('%s int data: photon number is %f',...
            data_int_print_cell{data_int_ind}, 2^(-pho_frac_ind)*10^8);
        tfw = 0;
        tbac = 0;
