...
%This file uses "sample_histogram" to use build a dataset with less photons
%than the recorded set (bootstrapped sample).

clear;
addpath('C:\Users\Bryan\Documents\MATLAB\FLIM')
load('lifetimes.mat');
load('intensity_mat.mat');
master_data{1} = load('cdata_low_100rep.mat');
master_data{2} = load('cdata_med_100rep.mat');
master_data{3} = load('cdata_hi_100rep.mat');

lifetimes_print_cell = {'low','med','high','NO_INT_CORR-lifetimes_from_low_int_data'};
data_int_print_cell = {'low','med','high'};
...
for lifetime_ind = 4:4 %1:length(dataname_cell)
    for data_int_ind = 1:1
        fprintf('\n\n%s int lifetime - %s int data\n',...
            lifetimes_print_cell{lifetime_ind},data_int_print_cell{data_int_ind}),
        for pho_frac_ind = 18:18
            jmax = 1;
            cyclesmax = 1;
            tseries = 0;
            exptmax= 1;
            [input] = ini_input(cyclesmax,exptmax,jmax); %Initialize input structure
            comment =  sprintf('%s int lifetime - %s int data: FF is %f',...
            lifetimes_print_cell{lifetime_ind},data_int_print_cell{data_int_ind}, 1/2^pho_frac_ind);
            tfw = 0;
            tbac = 0;
            
            %% Set search parameters
            if lifetime_ind <4
                w1step = .01; w1min= lifetimes.lifetime{lifetime_ind}; w1max = w1min;%1.73    %1.064-2016/2/27  --- %1.73  %1.6 used for extract 8/27 and 9/5 (actual 9/5 is 1.59) %1.5 used for taxol extract; %.8-2 1.05 %w1min must be an integer multiple of w1step.
                w2step = .001; w2min =  lifetimes.lifetime{lifetime_ind+3}; w2max = w2min;%3.802     %3.82/1.49   %3.8 used for 3/7/15 data %3.745 usd for 8/27 E %3.87 used for taxol extract; %3.81 was found for 8/25 b80 and 8/24 extract
                r1s = intensity.r1{data_int_ind}; r2s = intensity.r2{data_int_ind};
                r1l = intensity.r1{data_int_ind+3}; r2l = intensity.r2{data_int_ind+3};%intensity.r2{outerloop_ind};
            else %Setting the intensity and lifetimes for sl-no_int_corr and ll-no_int_corr
                w1step = .01; w1min= lifetimes.lifetime{7}; w1max = w1min;
                w2step = .001; w2min =  lifetimes.lifetime{8}; w2max = w2min;
                r1s = 1; r2s = 0;
                r1l = 1; r2l = 0;
            end
            ...
            for expt = 1:exptmax %determined by number of time series
                ...
                for cindex = 1:cyclesmax %determined by number of w2 spots
                    dataname = intensity.identifier{data_int_ind};
                    ...
                    while jind < jmax %j is determined by number of pixel groups
                        ...
                        jmax = 1;
                        p = master_data{data_int_ind}.data(pho_frac_ind,cindex).his;
                        p = p(tmini:tmaxi);
                        figure; plot(p);
                        ni = sum(p);
                        p = sample_histogram(p,'fraction',.1);
                        figure; plot(p);
                        ...
                        for save_input_mat = 1:1
                            ...
                            input(cindex,expt,jind).save_posterior_flag = 1;
